...
void FIRFilter::filterProcess(short* input, short* output, short len)
{
	...
	for(n = 0; n < len; n++){
		...
		*cir_ptr = x[n];
		...
		cir_ptr = (fract *)circptr(cir_ptr, sizeof(short), delayLine, sizeof(short)*filter_taps_);
		delay = (fract *)cir_ptr;

		for(m = 0; m < filter_taps_; m++){
			y_ += w[m] * (*delay);
			delay = (fract *)circptr(delay, sizeof(short), delayLine, sizeof(short)*filter_taps_);
		}
